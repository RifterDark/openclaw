...
- `--lobster-style image` keeps healthy state on native `🦞` and uses embedded recolored lobster PNGs (derived from the macOS lobster glyph, with yellow/white tones matched to `🟨` and `⬜`) for warning/critical via iTerm2's inline image protocol.
