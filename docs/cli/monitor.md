...
- `--lobster-style image` uses embedded recolored lobster PNGs (derived from the macOS lobster glyph, with yellow/white tones matched to `🟨` and `⬜`) via iTerm2's inline image protocol.
